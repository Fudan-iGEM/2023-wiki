...
In the pRAP system, there are two regulatory elements: the RBS and the stem loop. By regulating the intensity of these two regulatory elements, we can control the expression of proteins. RBS intensity influences translation initiation rate by influencing ribosome binding to mRNA, which in turn impacts protein concentration. Stem loop affects protein concentration by regulating the rate of mRNA degradation[^3]. For more details on building pRAP system, please refer to [RAP Builder](#rap-builder).
...
The need of user interface improvements were mentioned in several Learn phases of the DBTL cycles during the development of  RAP, so we end up  developing an intuitive but powerful Web UI for RAP. RAP's Web UI is based on [Vue.js](https://vuejs.org/), using [Ant Design Vue](https://antdv.com/docs/vue/introduce) as the front-end component library and [Axios](https://axios-http.com/) to access and process the APIs. [Neovis.js](https://github.com/neo4j-contrib/neovis.js) was used to visualize the network structure in PartHub 2.

Web UI of RAP has good compatibility and support the following browsers ([Table 1](#table-1)).
...
In this tutorial, it will be demonstrated how to use RAP to construct a pRAP sequence similar to [BBa_K4765121](http://parts.igem.org/Part:BBa_K4765121) or [BBa_K4765122](http://parts.igem.org/Part:BBa_K4765122). Click “see more“ below to get the information you need (Table S1). You can also directly access our pre-built [demo](http://54.169.242.254:5000/demo/) to generate the sequence.
...
1. **Visit Home Page of RAP**
   ...
   If you visit our [live demo](http://54.169.242.254:5000/home) or `Home` page of RAP deployed through docker, you will see the page below ([Figure 3](#fig-3)):
...
2. **Run Step 1: KineticHub**
   ...
   In order to search for the target enzyme more accurately, it is most recommended to use EC number to search for the enzyme. After typing in the query as well as selecting the keyword type and clicking `search`, you will see this interface ([Figure 4](#fig-4)):
   ...
   After clicking `Get Kcat`, the interface will look like this ([Figure 5](#fig-5)):
   ...
   After that, in the `KineticHub > Build reactions` page ([Figure 6](#fig-6)), enter the ratio of product to substrate stoichiometry for your reactions according to EC number and then click `Submit`.
...
3. **Run Step 2: RAP Builder**
   ...
   After running Step 1, you can run Step 2 to generate pRAP sequence. In `RAP Builder` page ([Figure 7](#fig-7)), you can select design mode (RBS design mode/Stem loop design mode) and input the sequence of the enzyme. Then you can click `Submit` to run RAP Builder! After running RAP Builder, the results will be automatically downloaded, containing a sequence file in `GeneBank` format and an annotation in `csv` format.
...
4. **Search Part in PartHub 2**
   ...
   To search Part with Parthub, you can go to `PartHub 2` page ([Figure 8](#fig-8)), input search query and select keyword type.
...
You can view the search results and see the Part you are interested in ([Figure 9](#fig-9)).
...
If you click on the search result, you will see the network structure of the Part. You can scroll to zoom the canvas and drag to move the nodes ([Figure 10](#fig-10)). Click to display the part details and double click to go to the part page. On this page, you can also download the GeneBank format sequence of this part directly.
...
We expect the optimal state of cellular metabolism to be flux-balanced, so we should take steady-state constraint into consideration, which can be expressed as:
